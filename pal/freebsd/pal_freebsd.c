...
#  error "pal/freebsd/pal_freebsd.c is currently only supported on x86-64."
#endif
...
static void *freebsd_mmap(size_t length)
{
    long ret = syscall6(197,
                        0L,
                        (long)length,
...
ssize_t pal_write(int fd, const void *buf, size_t count)
{
    ...
    return (ssize_t)syscall3(4, (long)fd, (long)buf, (long)count);
}

ssize_t pal_read(int fd, void *buf, size_t count)
{
    if (buf == NULL || count == 0) { return 0; }
    return (ssize_t)syscall3(3, (long)fd, (long)buf, (long)count);
}
